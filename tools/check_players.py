...
def main() -> None:
    ...
    parser.add_argument("--report", default="data_validation_report.txt", help="输出报告的文件名")
    ...
    for student_id, names in student_id_map.items():
        if len(names) > 1:
            ...
            errors.append(f"[学号冲突] 发现多个姓名使用同一学号 \"{student_id}\": {name_list}")
    ...
    for key_name, info in players.items():
        ...
        if inner_name != key_name:
            errors.append(f"[键名不一致] {key_name}: 顶层键名与内部name字段不一致 (name={inner_name})")
    ...
    for name, info in player_info.items():
        ...
        player_events[name] = build_event_index([e for e in registered_events if isinstance(e, dict)])
    ...
    for name, info in player_info.items():
        ...
        for event in registered_events:
            if not isinstance(event, dict):
                errors.append(f"[字段类型错误] {name}: registered_events 中存在非字典元素")
                continue
            ...
            if "单打" in event_type and partner:
                errors.append(f"[逻辑错误] {name}: 报名了\"{event_type}\"，却填写了搭档\"{partner}\"")
            if "双打" in event_type:
                if not partner:
                    errors.append(f"[逻辑错误] {name}: 报名了\"{event_type}\"，未填写搭档")
                elif partner == name:
                    errors.append(f"[逻辑错误] {name}: 报名了\"{event_type}\"，搭档不能是自己")

            # 搭档存在性与双向奔赴校验
            if partner:
                if partner not in player_info:
                    errors.append(f"[搭档不存在] {name}: 在\"{event_type}\"的搭档\"{partner}\"不在总名单中")
                    continue
                ...
                if "双打" in event_type:
                    ...
                    if not reciprocal_events:
                        errors.append(f"[搭档单相思] {name}: 搭档填了\"{partner}\"，但TA根本没报\"{event_type}\"")
                    else:
                        matched = any(safe_get_str(e, "partner") == name for e in reciprocal_events)
                        if not matched:
                            errors.append(f"[搭档劈腿] {name}: 搭档填了\"{partner}\"，但TA的搭档却不是你")

            # 性别合规性校验
            if "男子" in event_type and sex != "男":
                errors.append(f"[性别不符] {name}: 报名\"{event_type}\"，但登记性别为\"{sex}\"")
            ...
            if partner and partner in player_info:
                ...
                if "男子" in event_type and partner_sex != "男":
                    errors.append(f"[搭档性别不符] {name}: 你的搭档\"{partner}\"登记性别为\"{partner_sex}\"")
                if "女子" in event_type and partner_sex != "女":
                    errors.append(f"[搭档性别不符] {name}: 你的搭档\"{partner}\"登记性别为\"{partner_sex}\"")
                if "混合" in event_type and sex and partner_sex and sex == partner_sex:
                    errors.append(f"[混双性别冲突] {name}: 你与搭档\"{partner}\"的登记性别同为\"{sex}\"")
